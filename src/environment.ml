...
let environment_add_funk (env: environment) (funk: string) : environment =
  let (map, var_offset, param_offset) = env in
  (StringMap.add funk (XLabelLocationOffset("closure_of_"^funk, 1)) map), (var_offset), (param_offset)
...
(*A function to tell us the number of parameters of the current function, used in determining whether we can tail call optimize*)
let environment_get_num_params (env:environment):int =
    let (map, var_offset, param_offset) = env in
  param_offset
;;


let create_program_environment (declarations: a_declaration list): environment =
  let env = empty_environment in
  List.fold_left (fun env ad -> let AFunction(funk, _, _) = ad in environment_add_funk env funk) env declarations

;;
(*This one is for when we have a function, ergo params*)
let create_function_environment (params: string list) (declarations: a_declaration list): environment =
  let env = empty_environment in
  let env_with_params = List.fold_left environment_add_param env params in
  List.fold_left (fun env ad -> let AFunction(funk, _, _) = ad in environment_add_funk env funk) env_with_params declarations
